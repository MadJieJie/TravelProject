...
import static com.fengjie.myapplication.modules.run.base.NoteConstant.EXCEPTION_ID;
...
//		}
	...
	{
		Note note = NoteDao.getQueryInfoForDB(mID);
		title_et_newNote.setText(note.getTitle());
		content_et_newNote.setText(note.getContent());
		saveTime_tv_newNote.setText(note.getCreate_time());
	}
		...
		switch ( item.getItemId() )
		{
			case R.id.action_new_save:          //保存
				if ( mMode.equals(EDIT_MODE) )          //传入对象进行更新数据
				{
					...
					note.setUpdate_time(Time.getNowYMDHMSTime());
					NoteDao.updateDataToDB(note);
					ToastUtils.showShort(BaseApplication.sAppContext, "已经更新笔记");
					finish();           //退出界面
				} else if(mMode.equals(NEW_NOTE_MODE))                              //传入对象新建数据
				{
					if ( NoteDao.insertDataToDB(
							new Note(title_et_newNote.getText().toString(),
									        content_et_newNote.getText().toString(),
									        Time.getNowYMDHMSTime(), "")) )
					{
